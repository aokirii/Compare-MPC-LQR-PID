...
clc; close all;
clear persistent_anim_update nearest_ref_point mpc_solver
% NOT: 'clear all' KULLANMA -> diger kontrolcilerin sim_log_* loglari silinir.

%% --- Harita: KareMap (sabit, uc kontrolcude ortak) ---
...
L         = 2.0;
lr        = 1.0;
v_const   = 8;   % ortak hiz (PID/LQR ile ayni)
delta_max = deg2rad(30);
ddelta_max_per_step = 1.0; 

%% --- Sim parametreleri ---
Ts       = 0.02;
goal_tol = 1.0;
n_laps   = 3;        % ortak tur sayisi
% Tam 3 turluk sure (KareMap, v=8 -> ~120 s; 250 s ust sinir).
% Marj YOK: MPC lap-sayaci 3. turda delta=0 yapip duz kacmadan sim biter.
T_end    = min(250, (sU(end)/v_const)*n_laps);
...
nx  = 2;
nu  = 1;
%% --- Hata modeli (NOMINAL: sifir-nokta lineerizasyon) ---
%  NOT: Calisma-zamani modeli mpc_solver.m icinde A1 (her adim Jacobian) ile
%  guncel (eh, u_prev) etrafinda yeniden lineerize edilir. Buradaki sabit
%  Ac/Bc/Ec yalnizca terminal maliyet (dlqr -> P_term) icindir.
Ac = [0, v_const;
      0, 0];
Bc = [0;
      v_const/L];
...
assignin('base','R',             R);   % A1: solver H'yi her adim yeniden kurar
